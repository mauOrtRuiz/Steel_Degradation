...
The progressive degradation of microstructural low-carbon steel after long-term service is determined by spheroidization observed by scanning electron microscopy (SEM). While traditional computer vision techniques struggle to accurately quantify these complex morphological evolutions, deep learning offers robust alternatives. In this Study we present a comprehensive comparative evaluation of three computational approaches to assess progressive damage induced by accelerated artificial degradation via ultraviolet (UV) light exposure across four exposure intervals (0h, 500h, 1000h and 1500h) captured via Scanning Electron Microscopy (SEM). The methods under study are: classical deterministic 2D Fractal Dimension (D) analysis via Box-Counting, a standard baseline U-Net semantic segmentation model, a physics-conditioned U-Net FiLM architecture, our novel dual modulation-demodulation U-Net FiLM+D framework, and a ResNet-50 deep convolutional neural network leveraging macro-level soft-voting probability inference.

Evaluated on an independent blind test dataset (N=15), the classical Fractal Dimension approach demonstrated severe topological limitations, yielding a random-level accuracy of 20.00% (F1-score = 19.84%) and failing to distinguish intermediate spheroidization stages due to overlapping edge-density signatures. The U-Net segmentation framework achieved a moderate accuracy of 66.67% (F1-score = 65.00%), successfully isolating pearlite regions but showing sensitivity to local morphological non-linearities. In contrast, the ResNet-50 architecture achieved flawless performance with $100.00\%$ classification accuracy and macro F1-score, demonstrating its ability to extract rich, multi-scale hierarchical texture patterns that transcend local boundary noise. These results quantitatively demonstrate the inadequacy of first-order geometric metrics for thermal degradation assessment and highlight deep holistic feature extraction as a superior paradigm for automated metallographical inspection.
...
Approach 1: U-Net Semantic Segmentation and Morphological Thresholding. A U-Net architecture was trained to perform semantic segmentation of the microstructural phases. 
Also, a modified U-Net was evaluated, a Feature-wise Linear Modulation framework incorporating a joint modulation-demodulation scheme (FiLM+D) was evaluated for the segmentation task. In this model a multimodal network is traing by adding the parameter T, which modulates the backbone branch of the U-Net. In this architecture, an auxiliary multi-layer perceptron predicted channel-wise scaling (gamma) and shifting (beta) vectors to dynamically modulate intermediate feature maps prior to non-linear activation. The main idea behing this model is that each image processed during the segmentation has an associated time degradation value T, which is also used as a modulation parameter of the backbone.

<img width="456" height="74" alt="image" src="https://github.com/user-attachments/assets/f832b5dc-d1b1-404d-8052-87b4a3ed5495" />


A novel dual modulation-demodulation architecture modifying the standard Feature-wise Linear Modulation framework was proposed and tested for the segmentation task. The key contribution of the proposed FiLM+D model lies in the integration of an inverse Demodulation Head (+D) designed specifically to estimate the continuous degradation parameter T directly from unannotated SEM imagery. While the forward modulation branch uses a secondary multi-layer perceptron to compute channel-wise scale (gamma) and shift (beta) vectors from T to condition intermediate feature maps, the inverse demodulation head operates in reverse: it decodes the modulated latent features to infer and output the estimated exposure time T. This joint training scheme creates a closed loop where physical parameter conditioning enhances feature representation, enabling the network to regress T as a continuous output. While the inverse demodulation regression head exhibited instability when predicting the continuous scalar T on unannotated test samples, the feature modulation mechanism proved exceptionally effective for spatial feature extraction. By dynamically amplifying activation responses corresponding to spheroidal geometries while suppressing background noise, the FiLM-enhanced backbone achieved superior phase segmentation accuracy, outperforming the baseline U-Net model in isolating fine morphological boundaries as detailed in the comparative performance evaluation. However, because semantic segmentation alone relies on deterministic percentage cutoffs that remain sensitive to global threshold boundaries, unconstrained holistic feature extraction via ResNet-50 was ultimately adopted for the macro degradation time classification task.

<img width="854" height="256" alt="image" src="https://github.com/user-attachments/assets/accb1b1c-daf4-4e14-95e3-a3f29fe4153b" />


Following model convergence:The segmented masks were processed to extract morphological metrics, specifically quantifying the relative phase area fraction (%F) of degraded regions and lamellar structures. A rigorous statistical analysis was conducted on the distribution of %F across exposure intervals (0h to 1500h) to establish quantitative cutoff thresholds (decision boundaries). Degradation time was assigned deterministically based on these empirical percentage thresholds. The cut off values established are
...
* A Novel Curation and Annotation of an SEM Microstructural Dataset for UV Degradation: Creation and standardization of a specialized Scanning Electron Microscopy (SEM) dataset depicting progressive thermal and photochemical degradation across multi-stage accelerated UV exposure intervals ($0\text{ h}$, $500\text{ h}$, $1,000\text{ h}$, and $1,500\text{ h}$), providing a structured benchmark for material degradation segmentation and classification tasks.
* Design of the FiLM+D Architecture for High-Precision Morphological Segmentation: Proposal of FiLM+D, a novel dual modulation-demodulation architecture that integrates physical parameter conditioning ($\gamma, \beta$) with feature extraction, achieving superior phase segmentation performance on fine microstructural boundaries compared to standard baseline models.
* ResNet-50 Ensemble Framework with Localized Degradation Heatmaps: Development of a highly robust macro-level classification pipeline using ResNet-50 with patch-based soft-voting inference that achieves $100.00\%$ overall degradation stage accuracy, while simultaneously generating spatial resolution heatmaps to resolve intra-sample degradation heterogeneities and localized wear within individual micrographs.
* Comprehensive Benchmark Between Classical Descriptors and Deep Pipelines: (Added contribution) A rigorous quantitative benchmark demonstrating the severe topological limitations of first-order geometric metrics (Box-Counting Fractal Dimension, $20.00\%$ accuracy) against deep feature extraction paradigms for characterizing complex, non-linear microstructural damage kinetics.

**Work to do**

Building upon the insights and architectural foundations established in this study, several promising avenues remain for further investigation. A primary objective is the development and validation of a unified neural framework capable of estimating the exposure parameter $T$ as a continuous scalar function rather than restricting inference to discrete operational intervals. Transitioning from multi-class categorization to continuous regression will enable real-time prediction of intermediate material states, capturing the non-linear kinetics of thermal and photochemical degradation with fine-grained temporal resolution. Furthermore, expanding the experimental dataset to encompass SEM micrographs acquired across diverse spatial resolutions, tilt angles, and magnification levels will be crucial for assessing and strengthening cross-scale model transferability. Incorporating multiscale imagery will test the scale-invariance of the deep feature representations and ensure robust performance when deployed across varied laboratory setups and microscope configurations. Finally, future iterations of the architecture will focus on extending the multimodal conditioning scheme beyond exposure time. Integrating heterogeneous domain variables—such as operational surface temperatures, mechanical hardness measurements, local stress tensors, and chemical composition profiles—directly into the latent feature extraction pipeline will foster a holistic, physics-informed computational paradigm capable of modeling complex material degradation phenomena under real-world environmental conditions.
